...
/**
 * UUID MIGRATION SCRIPT - Birkenfeld Farm Security Upgrade (FIXED)
 * 
...
async function migrateToUUID() {
  console.log('🔒 STARTING UUID MIGRATION - BIRKENFELD FARM SECURITY UPGRADE (FIXED)');
  console.log('=====================================================================\n');
  ...
  try {
    ...
    console.log(`   🏷️  Categories: ${birkenfeldTenant.categories.length}`);
    console.log('');
    ...
    testTenants.forEach(tenant => {
      ...
      console.log(`      🏷️  Categories: ${tenant.categories.length}`);
    });
    console.log('');
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Step 5: Delete test tenants (with proper foreign key handling)
      console.log('🗑️  Step 5: Deleting test tenants...');
      let totalDeleted = 0;
      
      for (const testTenant of testTenants) {
        console.log(`   🗑️  Deleting ${testTenant.name}...`);
        
        // Delete categories first (foreign key constraint)
        const categoryDeleteResult = await tx.category.deleteMany({
          where: { tenantId: testTenant.id }
        });
        console.log(`      🏷️  Deleted ${categoryDeleteResult.count} categories`);
        
        // Delete users (foreign key constraint)
        ...
        // Delete items (if any)
        const itemDeleteResult = await tx.stolenItem.deleteMany({
          where: { tenantId: testTenant.id }
        });
        console.log(`      📦 Deleted ${itemDeleteResult.count} items`);

        // Delete tenant
        await tx.tenant.delete({
          where: { id: testTenant.id }
        });
        console.log(`      ✅ Deleted tenant: ${testTenant.name}`);
      ...
      return {
        ...
        testTenantsDeleted: totalDeleted
      };
    });
